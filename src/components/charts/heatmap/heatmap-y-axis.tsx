...
export interface HeatmapYAxisProps {
  ...
  /** Custom row labels (e.g. category names). Overrides weekday labels and shows every row. */
...
export const HeatmapYAxis = memo(function HeatmapYAxis({
  ...
  const labels = useMemo(() => {
    const source = customLabels ?? getHeatmapDayLabels(weekStartDay);
    const ticks = source.map((label, row) => ({
      row,
      label: customLabels ? label : formatHeatmapYAxisLabel(label, labelFormat),
      y: margin.top + yScale(row) + (binHeight - gap) / 2,
    }));
    // Custom category labels always show every row.
    return customLabels
      ? ticks
      : ticks.filter((tick) => shouldShowHeatmapYAxisTick(tick.row, tickFilter));
  }, [
    binHeight,
    customLabels,
    gap,
