...
function AnimatedBar({
  ...
  const initial = isHorizontal
    ? { width: 0, height: safeHeight, x: 0, y }
    : { width: safeWidth, height: 0, x, y: innerHeight };
  const target = isHorizontal
    ? { width: safeWidth, height: safeHeight, x: 0, y }
    : { width: safeWidth, height: safeHeight, x, y };
  ...
  return (
    <g className={`bar-series-${uniqueId}`}>
      {data.map((d, i) => {
        ...
        barHeight = Math.max(0, barHeight || 0);
        barW = Math.max(0, barW || 0);
