...
const ChartCore = memo(function ChartCore({
  ...
  // Create a scale function for compatibility with ChartContext & XAxis/YAxis
  const fakeTimeScale = useMemo(() => {
    const fn = (val) => categoryScale(val) ?? 0;
    fn.domain = () => categoryScale.domain();
    fn.range = () => categoryScale.range();
    fn.bandwidth = () =>
      categoryScale.bandwidth ? categoryScale.bandwidth() : 0;
    fn.step = () => (categoryScale.step ? categoryScale.step() : 0);
    fn.copy = () => fn;
    return fn;
  }, [categoryScale]);
  ...
  const contextValue = {
    ...
    data,
    renderData: data,
    xScale: isHorizontal ? valueScale : fakeTimeScale,
