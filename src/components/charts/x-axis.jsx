...
function buildDomainTicks(
  ...
  if (typeof xScale.ticks === "function") {
    const rawTicks = xScale.ticks(numTicks);
    const seenLabels = new Set();
    const ticks = [];

    for (const val of rawTicks) {
      let label = "";
      if (val instanceof Date) {
        label = shortDateFmt.format(val);
      } else if (typeof val === "number" && val > 100000000000) {
        label = shortDateFmt.format(new Date(val));
      } else {
...
function domainExtendsPastData(data, xAccessor, xScale) {
  if (!data || data.length === 0 || !xScale?.domain) {
    return false;
  }
  ...
  if (typeof domainEnd?.getTime === "function" && typeof lastDate?.getTime === "function") {
    return domainEnd.getTime() > lastDate.getTime();
  }
  if (typeof domainEnd === "number" && typeof lastDate === "number") {
    return domainEnd > lastDate;
  }
  return false;
}
...
function appendProjectionTailTicks(ticks, data, xAccessor, xScale, marginLeft, maxExtraTicks) {
  if (!data || data.length === 0 || maxExtraTicks <= 0 || !xScale?.domain) {
    return ticks;
  }
  ...
  const lastDate = xAccessor(lastPoint);
  if (typeof lastDate?.getTime !== "function" || typeof domainEnd?.getTime !== "function") {
    return ticks;
  }
...
const XAxisInner = memo(function XAxisInner({
  ...
  const labelsToShow = useMemo(() => {
    // If data exists, prefer data-aligned ticks to prevent sub-day duplicate string labels
    if (data && data.length > 0) {
      ...
      if (dataTicks && dataTicks.length > 0) {
        return dataTicks;
      }
    }
  ...
  if (isHovering && tooltipData) {
    const rawVal = dateLabels[tooltipData.index] ?? (tooltipData.point ? xAccessor(tooltipData.point) : null);
    if (rawVal instanceof Date) {
      hoveredLabel = shortDateFmt.format(rawVal);
    } else if (rawVal != null) {
      hoveredLabel = String(rawVal);
    }
  }
  ...
  return createPortal(<div className="pointer-events-none absolute inset-0">
    {labelsToShow.map((item, i) => (
      <XAxisLabel
        ...
        key={`${item.date?.getTime ? item.date.getTime() : (item.label || item.val || i)}-${item.x}-${i}`}
        label={item.label}
