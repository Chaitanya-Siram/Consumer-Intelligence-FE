...
const DateTickerCompact = memo(function DateTickerCompact({
  ...
  return (
    <div
      className="overflow-hidden rounded-full bg-white px-4 py-1 text-zinc-900 shadow-[0_6px_24px_rgba(0,0,0,0.18)] border border-zinc-200/80 dark:bg-zinc-900 dark:text-zinc-100 dark:border-zinc-800">
...
export function DateTicker({
  ...
  if (!visible || !labels || labels.length === 0) {
    return null;
  }

  const isDateLabels = useMemo(() => {
    if (!labels.length) return false;
    const sample = String(labels[0] || "").trim();
    const firstWord = sample.split(" ")[0];
    return MONTHS.has(firstWord);
  }, [labels]);

  if (!isDateLabels || labels.length > COMPACT_TICKER_THRESHOLD) {
