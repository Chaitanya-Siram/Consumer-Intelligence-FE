...
export function Area({
  ...
  const safeUniqueId = String(uniqueId).replace(/[^a-zA-Z0-9_-]/g, "_");
  const gradientId = `area-gradient-${safeDataKey}-${safeUniqueId}`;
  const strokeGradientId = `area-stroke-gradient-${safeDataKey}-${safeUniqueId}`;
  const edgeMaskId = `area-edge-mask-${safeDataKey}-${safeUniqueId}`;
