...
export const LAYOUTS = ['Sense', 'Classic', 'Editorial', 'Merger', 'PR Impact', 'Glass', 'Bento']

import { updateApiPayload } from "./workflowUtils.js";
...
export function defaultNodeData(type, extra = {}) {
  switch (type) {
    case 'data': {
      ...
      if (dataObj.sourceType === 'api') {
        const queryText = dataObj.query || (dataObj.brandKeywords || []).join(", ");
        // Selected providers live in data_sources; a bare boolean here would
        // collapse them to google_news.
        const selected = dataObj.selectedSources?.length
          ? dataObj.selectedSources
          : dataObj.data_sources?.length
            ? dataObj.data_sources
            : ['google_news'];
        const { sources, queries } = updateApiPayload(
          queryText,
          selected,
          dataObj.brandKeywords,
        );
        dataObj.data_sources = sources;
        dataObj.queries = queries;
      }
      return dataObj;
    }
    case 'analysis':
    ...
    case 'output':
      return { label: 'Output', format: 'Dashboard', projectName: '', projectDescription: '', ...extra }
    default:
