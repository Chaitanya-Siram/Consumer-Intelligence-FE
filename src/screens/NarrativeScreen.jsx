...
import Template1 from "../dashboards/narrative_intelligence/Template1.jsx";
import Template2 from "../dashboards/narrative_intelligence/Template2.jsx";
import Template3 from "../dashboards/narrative_intelligence/Template3.jsx";
import Template4 from "../dashboards/narrative_intelligence/Template4.jsx";
import Template5 from "../dashboards/narrative_intelligence/Template5.jsx";
import Template6 from "../dashboards/narrative_intelligence/Template6.jsx";
import Template7 from "../dashboards/narrative_intelligence/Template7.jsx";

const DASHBOARD_KEY = "narrative_intelligence";

const TAB_HERO = {
  Overview: {
    label: "Narrative Overview",
    lead: "Active narratives,",
    em: "quantified.",
    sub: "Track consistent and stray messages across channels.",
  },
  Coverage: {
    label: "Coverage Trends",
    lead: "Daily volume,",
    em: "by competitor.",
    sub: "Spot narrative momentum and breakout press coverage.",
  },
  Sentiment: {
    label: "Sentiment Breakdown",
    lead: "Brand perception,",
    em: "under the lens.",
    sub: "Evaluate net sentiment splits across tracked companies.",
  },
  "Message Consistency": {
    label: "Message Consistency",
    lead: "Message alignment,",
    em: "measured over time.",
    sub: "How much of your coverage contains key narrative pillars.",
  },
  "Channels & Publications": {
    label: "Channels & Publications",
    lead: "Outlets and types,",
    em: "shaping the agenda.",
    sub: "Analyze which channels publish most and which media types lead.",
  },
};
...
export default function NarrativeScreen({
  ...
  const dynamicCharts = useMemo(() => {
    const fromBackend =
      ...
      chartsData?.narrative_intelligence_dynamic_charts ||
  ...
  const topNarratives = chartsData?.top_narratives || [];
  ...
  const heroFor = (name) => {
    if (name === "Overview") {
      ...
      return {
        kicker: `Narrative Intelligence · ${ch?.section_label || TAB_HERO.Overview.label}`,
        lead: `${nf(totalCount)} articles analysed.`,
        em: `Evaluating narrative alignment.`,
    ...
    if (ch && (ch.title || ch.description)) {
      ...
      return {
        kicker: `Narrative Intelligence · ${ch.section_label || TAB_HERO[name]?.label || name}`,
    ...
    return {
      kicker: `Narrative Intelligence · ${f.label}`,
      lead: f.lead,
      em: f.em,
      sub: f.sub,
    };
  ...
  // Brand selection state for publication charts
  ...
  useEffect(() => {
    if (brandsList.length > 0 && !brandsList.includes(selectedBrand)) {
      setSelectedBrand(brandsList[0]);
    }
  }, [brandsList, selectedBrand]);

  // Transformed chart objects for DynamicChartRenderer
  const selectedBrandChart = useMemo(() => {
    const chart = byId.publication_by_brands_and_competitors;
    if (!chart) return null;
    const brandData = chart.data?.[selectedBrand] || {};
    return {
      ...chart,
      data: Object.entries(brandData).map(([name, value]) => ({ name, value })),
    };
  }, [byId, selectedBrand]);
  ...
  const formattedCoverageChart = useMemo(() => {
    ...
    const out = {};
    Object.entries(brandsObj).forEach(([brand, dateObj]) => {
      Object.entries(dateObj || {}).forEach(([date, count]) => {
        if (!out[date]) out[date] = { date };
        out[date][brand] = count;
      });
    });
    ...
    <>
      ...
      {templateMode === "sense" ? (
        <Template7
          ...
          selectedBrandChart={selectedBrandChart}
          formattedCoverageChart={formattedCoverageChart}
          formattedSentimentChart={formattedSentimentChart}
          formattedConsistencyTimeChart={formattedConsistencyTimeChart}
      ...
      <ChatDock
        ...
        chartsData={chartsData}
        dashboardKey="narrative_intelligence"
        project={project}
        session={session}
      />
    </>
